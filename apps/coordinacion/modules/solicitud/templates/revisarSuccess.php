<?php 
	...
	var storeSolicitud = new Ext.data.JsonStore({
        url: '".URL_SASPA."coordinacion_dev.php/solicitud/lista',
        root: 'datos',
        totalProperty: 'total',
        fields: [
          {name:'sol_id'},
   	...
   	var record = gridConsulta.getSelectionModel().getSelected();
    	if(!Ext.isEmpty(record) && record.data.sol_estado == 'Revision'){
          ...
          fn      : function (btn){
            if(btn == 'yes'){
					Ext.Ajax.request({   
                url     : '".URL_SASPA."coordinacion_dev.php/solicitud/revisar',
                params  : {
    ...
    */
    function comentar()
    {
		 ...
		 if(!Ext.isEmpty(record) && record.data.sol_estado == 'Revision')
		 {
		 		Saspa.comentario.comentar('".URL_SASPA."coordinacion_dev.php/solicitud/comentarSolicitud',record.get('sol_id'),record.get('sol_nombre'));
		 }else{
		 		Ext.Msg.alert('Mensaje','Selecione una solicitud');
		 }
    }
