...
Saspa.comentario.gstcomentario = function(urlserv,sol_id)
{
	...
	var winprincipal = new Ext.Window({
		...
		tbar   : [
			{
				text    : 'Procesada', 
        		tooltip : 'Observacion procesada',
        		iconCls : 'x-btn-text-ico',
        		icon    : '../images/activos.png',
        		handler : procesarComentario
			},' ','-',
			{
				text    : 'No Procesada', 
        		tooltip : 'Observacion no procesada',
        		iconCls : 'x-btn-text-ico',
	...
	function procesarComentario(btn,evobj)
	{
		//Ext.Msg.alert('Mensaje',btn.getText());
		
		var rec = gridcentral.getSelectionModel().getSelected();
		if(!Ext.isEmpty(rec))
		{
			Ext.Ajax.request({   
		 		url    : urlserv,
				params : {
					coment    : rec.get('comid'),
					newEstado : btn.getText()
				},
			...
			Ext.Msg.alert('Mensaje','Selecione una observacion');
		}
	}
